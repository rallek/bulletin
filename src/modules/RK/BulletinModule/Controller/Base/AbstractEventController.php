...
abstract class AbstractEventController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('rkbulletinmodule_event_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'rkbulletinmodule_event_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
        
        $sortableColumns->addColumns([
            new Column('workflowState'),
            new Column('eventTitle'),
     ...
     * @ParamConverter("event", class="RKBulletinModule:EventEntity", options = {"id" = "id", "repository_method" = "selectById"})
    ...
    protected function displayInternal(Request $request, EventEntity $event, $isAdmin = false)
    {
        ...
        $featureActivationHelper = $this->get('rk_bulletin_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
            if (!$this->get('rk_bulletin_module.category_helper')->hasPermission($event)) {
                throw new AccessDeniedException();
            }
        }
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('rk_bulletin_module.form.handler.event');
    ...
    protected function deleteInternal(Request $request, EventEntity $event, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'RKBulletinModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'event', 'id' => $event->createCompositeIdentifier()];
        ...
        $redirectRoute = 'rkbulletinmodule_event_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        $form = $this->createForm('RK\BulletinModule\Form\DeleteEntityType', $event);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'RKBulletinModule', 'user' => $userName, 'action' => $action, 'entity' => 'event', 'id' => $itemId, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'RKBulletinModule', 'user' => $userName, 'entity' => 'event', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'RKBulletinModule', 'user' => $userName, 'action' => $action, 'entity' => 'event', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rkbulletinmodule_event_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('rkbulletinmodule_event_' . ($isAdmin ? 'admin' : '') . 'index');
    }
    ...
    public function handleInlineRedirectAction($idPrefix, $commandName, $id = 0)
    {
        ...
        return new PlainResponse($this->get('twig')->render('@RKBulletinModule/Event/inlineRedirectHandler.html.twig', $templateParameters));
    }
}
