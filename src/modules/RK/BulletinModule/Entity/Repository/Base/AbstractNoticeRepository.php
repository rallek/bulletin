 ...
 *
 * This is the base repository class for notice entities.
 */
abstract class AbstractNoticeRepository extends EntityRepository
{
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('RK\BulletinModule\Entity\NoticeEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'RKBulletinModule', 'entity' => 'notice'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('RK\BulletinModule\Entity\NoticeEntity', 'tbl')
        ...
        $logArgs = ['app' => 'RKBulletinModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'notices', 'userid' => $userId];
     ...
     *
     * @return array|noticeEntity retrieved data array or noticeEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved noticeEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryHelper = ServiceUtil::get('rk_bulletin_module.category_helper');
                $qb = $categoryHelper->buildFilterClauses($qb, 'notice', $v);
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        $parameters['searchTeaser'] = '%' . $fragment . '%';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\BulletinModule\Entity\NoticeEntity', 'tbl');
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        if (!empty($where)) {
            ...
            $categoryHelper = ServiceUtil::get('rk_bulletin_module.category_helper');
            $categoryProperties = $categoryHelper->getAllProperties('notice');
    ...
    public function getQueryFromBuilder(QueryBuilder $qb)
    {
        ...
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::TRANSLATIONS, 'notice')) {
