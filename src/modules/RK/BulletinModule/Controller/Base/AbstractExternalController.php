...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction($objectType, $id, $source, $displayMode)
    {
        ...
        $templateParameters['featureActivationHelper'] = $this->get('rk_bulletin_module.feature_activation_helper');
        
        return $this->render('@RKBulletinModule/External/' . ucfirst($objectType) . '/display.html.twig', $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@RKBulletinModule:css/style.css'));
        ...
        if (!$this->hasPermission('RKBulletinModule:' . ucfirst($objectType) . ':', '::', ACCESS_COMMENT)) {
            throw new AccessDeniedException();
        }
        ...
        $form = $this->createForm('RK\BulletinModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        $output = $this->renderView('@RKBulletinModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
