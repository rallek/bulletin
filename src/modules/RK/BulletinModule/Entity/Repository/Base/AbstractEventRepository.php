...
abstract class AbstractEventRepository extends EntityRepository
{
     ...
     */
    protected $defaultSortingField = 'eventTitle';
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters['notice'] = $this->getRequest()->query->get('notice', 0);
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('RK\BulletinModule\Entity\EventEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'RKBulletinModule', 'entity' => 'event'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('RK\BulletinModule\Entity\EventEntity', 'tbl')
        ...
        $logArgs = ['app' => 'RKBulletinModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'events', 'userid' => $userId];
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = [])
    {
        ...
        $isAdminArea = false !== strpos($routeName, 'rkbulletinmodule_event_admin');
        if ($isAdminArea) {
            return $qb;
        }
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            ...
            if ($showOnlyOwnEntries == 1) {
                // allow the owner to see his deferred events
        ...
        $qb->andWhere('tbl.startDate <= :startDate')
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        $filters[] = 'tbl.eventTitle LIKE :searchEventTitle';
        $parameters['searchEventTitle'] = '%' . $fragment . '%';
    ...
    protected function getCountQuery($where = '', $useJoins = false)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\BulletinModule\Entity\EventEntity', 'tbl');
     ...
     * @param integer $excludeId  Id of events to exclude (optional)
     *
    ...
    public function getQueryFromBuilder(QueryBuilder $qb)
    {
        ...
        $featureActivationHelper = ServiceUtil::get('rk_bulletin_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::TRANSLATIONS, 'event')) {
    ...
    {
        $qb->leftJoin('tbl.notice', 'tblNotice');
