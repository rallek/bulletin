...
/**
 * Notice controller base class.
 */
abstract class AbstractNoticeController extends AbstractController
{
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
            $templateParameters['items'] = $this->get('rk_bulletin_module.category_helper')->filterEntitiesByPermission($templateParameters['items']);
        }
     ...
     * @ParamConverter("notice", class="RKBulletinModule:NoticeEntity", options = {"id" = "id", "repository_method" = "selectById"})
     * @Cache(lastModified="notice.getUpdatedDate()", ETag="'Notice' ~ notice.getid() ~ notice.getUpdatedDate().format('U')")
     *
     * @param Request $request Current request instance
     * @param NoticeEntity $notice Treated notice instance
     *
     * @return Response Output
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown by param converter if notice to be displayed isn't found
     */
    public function adminDisplayAction(Request $request, NoticeEntity $notice)
    {
        return $this->displayInternal($request, $notice, true);
    }
     ...
     * @throws NotFoundHttpException Thrown by param converter if notice to be displayed isn't found
     */
    public function displayAction(Request $request, NoticeEntity $notice)
    {
        return $this->displayInternal($request, $notice, false);
    }
    
    /**
     * This method includes the common implementation code for adminDisplay() and display().
     */
    protected function displayInternal(Request $request, NoticeEntity $notice, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'notice';
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
        ...
        $instanceId = $notice->createCompositeIdentifier();
        ...
        $featureActivationHelper = $this->get('rk_bulletin_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
            if (!$this->get('rk_bulletin_module.category_helper')->hasPermission($notice)) {
                throw new AccessDeniedException();
            }
        }
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown by form handler if notice to be edited isn't found
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('rk_bulletin_module.form.handler.notice');
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown by param converter if notice to be deleted isn't found
     * @throws RuntimeException      Thrown if another critical error occurs (e.g. workflow actions not available)
     */
    public function adminDeleteAction(Request $request, NoticeEntity $notice)
    {
        return $this->deleteInternal($request, $notice, true);
    }
    ...
    {
        return $this->deleteInternal($request, $notice, false);
    }
    
    /**
     * This method includes the common implementation code for adminDelete() and delete().
     */
    protected function deleteInternal(Request $request, NoticeEntity $notice, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'notice';
        ...
        $logArgs = ['app' => 'RKBulletinModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'notice', 'id' => $notice->createCompositeIdentifier()];
        
        $notice->initWorkflow();
        
        // determine available workflow actions
        $workflowHelper = $this->get('rk_bulletin_module.workflow_helper');
        $actions = $workflowHelper->getActionsForObject($notice);
        ...
        $redirectRoute = 'rkbulletinmodule_notice_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this notice.'));
            $logger->error('{app}: User {user} tried to delete the {entity} with id {id}, but this action was not allowed.', $logArgs);
        
            return $this->redirectToRoute($redirectRoute);
        }
        
        $form = $this->createForm('RK\BulletinModule\Form\DeleteEntityType', $notice);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                $hookHelper = $this->get('rk_bulletin_module.hook_helper');
                // Let any hooks perform additional validation actions
                $validationHooksPassed = $hookHelper->callValidationHooks($notice, 'validate_delete');
                if ($validationHooksPassed) {
                    // execute the workflow action
                    $success = $workflowHelper->executeAction($notice, $deleteActionId);
                    if ($success) {
                        $this->addFlash('status', $this->__('Done! Item deleted.'));
                        $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', $logArgs);
                    }
                    
                    // Let any hooks know that we have deleted the notice
                    $hookHelper->callProcessHooks($notice, 'process_delete', null);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemid) {
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rkbulletinmodule_notice_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
